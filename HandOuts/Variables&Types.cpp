...
int main()
{
    ...
    cout<<c<<NL<<d<<NL;
    // هرچند تا سرریز داشته باشه به همون مقدار دوباره از صفر شروع میکنه(OverFlow)
    // NeonLearn S4 T1:03:37
    ...
    int initialize4{4};     // uniform initialization (more efficient in complex DataTypes)
        /*
            ...
            // 3 ways to Initialization :
               int initialize{} //(higher performance)
               int initialize{value}
               int initialize = {value or values}
        */
